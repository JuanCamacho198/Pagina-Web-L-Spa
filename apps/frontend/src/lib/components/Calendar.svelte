...
<div class="p-4 bg-white rounded-3xl shadow-xl border border-gray-100 max-w-sm mx-auto">
  <div class="flex items-center justify-between mb-6">
    ...
    <div class="flex gap-2">
      <button
        onclick={prevMonth}
        class="h-10 w-10 flex items-center justify-center rounded-2xl hover:bg-gray-50 border border-gray-100 transition-all active:scale-90"
      >
  ...
  <div class="grid grid-cols-7 gap-1">
    ...
    {#each Array(totalDays) as _, i}
      ...
      <button
        ...
        disabled={disabled}
        class={cn(
          "h-10 w-10 rounded-2xl flex items-center justify-center text-sm transition-all font-medium relative focus:outline-none",
          selected 
