...
<style>
	:global(body) {
        ...
        background-attachment: fixed;
								...
								<span class="text-[10px] font-black uppercase tracking-widest text-gray-400">Impuestos</span>
								<span class="text-lg font-black text-emerald-600">Incluidos</span>
							</div>
							
							<div class="h-1.5 w-full bg-gray-50 rounded-full my-8"></div>
							
							<div class="flex flex-col gap-2">
								<span class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 text-center">Total a pagar</span>
								<span class="text-6xl font-black text-gray-900 text-center leading-none tracking-tighter">${total.toLocaleString()}</span>
							</div>
						</div>
						...
						<div class="space-y-4 relative z-10">
							<Button
								href="/checkout"
								class="w-full py-6 rounded-[28px] font-black text-sm uppercase tracking-[0.2em] shadow-2xl shadow-primary/20 hover:shadow-primary/40 active:scale-95 transition-all duration-300 group"
							>
								Finalizar Reserva
								<CreditCard size={20} class="ml-3 group-hover:animate-bounce" />
							</Button>
							
							<div class="flex items-center gap-4 bg-emerald-50 p-4 rounded-2xl border border-emerald-100/50">
								<ShieldCheck class="text-emerald-500 shrink-0" size={20} />
								<p class="text-[9px] font-black uppercase tracking-widest text-emerald-800 leading-tight">
									Reserva 100% Segura • Pago Cifrado SSL
								</p>
							</div>
						</div>

						<div class="mt-12 flex flex-col items-center gap-4 opacity-50 relative z-10">
							<span class="text-[9px] font-black uppercase tracking-[0.3em] text-gray-400">Aceptamos</span>
							<div class="flex items-center justify-center gap-6 grayscale scale-90">
					...
					<div class="mt-8 grid grid-cols-2 gap-4">
						<div class="bg-white/50 p-4 rounded-3xl border border-gray-100 flex items-center gap-3">
							<div class="h-8 w-8 rounded-xl bg-primary/5 flex items-center justify-center text-primary">
								<Info size={16} />
							</div>
							<span class="text-[8px] font-black uppercase tracking-wider text-gray-500">Cancelación Flexible</span>
						</div>
						<div class="bg-white/50 p-4 rounded-3xl border border-gray-100 flex items-center gap-3">
							<div class="h-8 w-8 rounded-xl bg-primary/5 flex items-center justify-center text-primary">
								<Sparkles size={16} />
							</div>
							<span class="text-[8px] font-black uppercase tracking-wider text-gray-500">Atención Premium</span>
						</div>
					</div>
				</div>
			</div>
