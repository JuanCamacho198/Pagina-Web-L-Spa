	...
	{#if loading && services.length === 0}
		...
		<div class="flex items-center justify-center py-20">
			<div class="flex flex-col items-center gap-4">
				...
				<p class="text-gray-500 font-medium">Cargando servicios...</p>
			</div>
		</div>
	{:else}
		<!-- Filters -->
			...
			{#each filteredServices as service}
				<div class="bg-white rounded-4xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-shadow {service.active ? '' : 'opacity-60'}">
					...
					<div class="relative h-48 overflow-hidden">
						...
						<div class="absolute top-4 right-4 flex items-center gap-2">
							<button 
								...
								class="p-2 bg-white/90 rounded-xl shadow-lg hover:bg-white transition-colors disabled:opacity-50"
							>
					...
					<div class="p-6">
						...
						<p class="text-gray-500 text-sm mb-6 line-clamp-2">{service.description || 'Sin descripción'}</p>
						...
						<div class="flex items-center justify-between pt-4 border-t border-gray-100">
							...
							<div class="flex items-center gap-2">
								...
								<button 
									...
									disabled={actionLoading === service.id}
									class="p-2 text-gray-400 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-colors disabled:opacity-50" 
									title="Eliminar"
								>
									<Trash2 size={18} />
								</button>
...
{#if showModal}
	<div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
		<div class="bg-white rounded-4xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
			<!-- Header -->
			...
			<div class="p-6 space-y-4">
				<div>
					<label for="name" class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Nombre</label>
					<input 
						id="name"
						type="text" 
				...
				<div>
					<label for="description" class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Descripción</label>
					<textarea 
						id="description"
						bind:value={formData.description}
						rows="3"
						class="w-full px-6 py-4 rounded-2xl border border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all font-medium resize-none"
				...
				<div class="grid grid-cols-2 gap-4">
					<div>
						<label for="price" class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Precio (COP)</label>
						<input 
							id="price"
							type="number" 
					...
					<div>
						<label for="duration" class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Duración (min)</label>
						<input 
							id="duration"
							type="number" 
				...
				<div>
					<label for="category" class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Categoría</label>
					<select 
						id="category"
						bind:value={formData.category}
						class="w-full px-6 py-4 rounded-2xl border border-gray-200 focus:border-primary outline-none font-medium bg-white"
					>
						{#each categories as category}
							<option value={category}>{getCategoryLabel(category)}</option>
						{/each}
				...
				<div>
					<label for="imageUrl" class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">URL de Imagen</label>
					<input 
						id="imageUrl"
						type="url" 
				...
				<label class="flex items-center gap-3 cursor-pointer">
					...
					/>
					<span class="font-medium text-gray-900">Servicio activo</span>
				</label>
			</div>
			...
			<div class="flex items-center justify-end gap-4 p-6 border-t border-gray-100">
				<button 
					onclick={() => showModal = false}
					class="px-6 py-3 text-gray-600 font-medium hover:bg-gray-100 rounded-2xl transition-colors"
				>
					Cancelar
				</button>
				<button 
