	...
	<div class="max-w-7xl mx-auto px-6 lg:px-8">
		...
		{#if error}
			<div class="bg-white rounded-[40px] p-20 text-center shadow-2xl shadow-rose-100/50 border border-rose-50 max-w-2xl mx-auto animate-in fade-in zoom-in duration-500">
				...
				<Button onclick={() => window.location.reload()} class="rounded-4xl px-10 py-5">Intentar de nuevo</Button>
			</div>
					...
					<div class="bg-white rounded-[40px] overflow-hidden border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col group self-start hover:-translate-y-2">
						<div class="p-10 grow">
							<div class="flex justify-between items-start mb-8">
								<Badge class={cn("rounded-4xl px-4 py-2 font-black uppercase tracking-widest text-[10px] border-none shadow-sm", status.color)}>
									{status.label}
								</Badge>
...
>
	...
	{#snippet footer()}
		<Button variant="ghost" onclick={() => (modalId = null)} class="rounded-4xl font-black uppercase tracking-widest">No, mantener</Button>
		<Button onclick={handleCancel} isLoading={cancelingId !== null} class="rounded-4xl px-8 bg-rose-500 hover:bg-rose-600 shadow-rose-200 font-black uppercase tracking-widest">Sí, cancelar</Button>
	{/snippet}
</Modal>
